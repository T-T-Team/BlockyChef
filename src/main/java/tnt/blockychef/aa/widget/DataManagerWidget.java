...
public class DataManagerWidget<SRC> extends ContainerWidget {
    ...
    public DataManagerWidget(int pX, int pY, int pWidth, int pHeight, DataManagerProperties<SRC> properties, List<SRC> data) {
        ...
        this.dataManager = new DataManager<>(properties.view(), data).withDataChangeListener((adjustedData, dataview) -> {
            ...
            if (filteringEnabled) {
                ...
                if (filters.size() > 0) {
                    ...
                    for (Filter<SRC> filter : filters) {
                        ...
                        addRenderableWidget(factory.createGuiWidget(left, startY, width, 20, () -> setView(dataview)));
            ...
            if (sortingEnabled) {
                ...
                if (availableSorters.size() > 0 && sorters.size() < availableSorters.size()) {
                    ColorButton button = addRenderableWidget(new ColorButton(pX + labelSize + 10, startY, 60, 20, ADD_SORTER_LABEL, btn -> {}));
                    //button.setPrimaryColorSchema(0, 0, 0);
                    //button.setSecondaryColorSchema(0, 0, 0);
                }
