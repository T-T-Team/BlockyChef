...
public class JuicerRecipe extends AbstractFoodRecipe<JuicerBlockEntity> {
    ...
    public static final CodecRecipeSerializer.CodecProvider<JuicerRecipe> CODEC_PROVIDER = id -> RecordCodecBuilder.create(instance -> instance.group(
    ...
    ).apply(instance, (in, amount, fluid, exp, cons) -> new JuicerRecipe(id, in, amount, fluid, exp, cons)));

    private final Ingredient input;
    private final int pressAmount;
    private final FluidStack output;

    public JuicerRecipe(ResourceLocation id, Ingredient ingredient, int pressAmount, FluidStack value, float experience, List<MultiIngredient> remainderConsumer) {
