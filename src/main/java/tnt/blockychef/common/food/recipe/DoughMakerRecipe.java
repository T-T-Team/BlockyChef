...
public class DoughMakerRecipe extends AbstractFoodRecipe<DoughMakerBlockEntity> {
    ...
    public static final CodecRecipeSerializer.CodecProvider<DoughMakerRecipe> CODEC_PROVIDER = id -> RecordCodecBuilder.create(instance -> instance.group(
    ...
    ).apply(instance, (in, out, time, exp, cons) -> new DoughMakerRecipe(id, in, out, time, exp, cons)));

    private final List<MultiIngredient> inputs;
    private final ItemStack[] outputs;
    private final int processingTime;

    public DoughMakerRecipe(ResourceLocation id, List<MultiIngredient> inputs, ItemStack[] outputs, int processingTime, float experience, List<MultiIngredient> remainderConsumer) {
